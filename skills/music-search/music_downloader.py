...
def _emit_dashboard(kind: str, text: str):
    """直接写入 agent_events.jsonl 上报到 Dashboard（不依赖 Agent 上报）"""
    try:
        import datetime as _dt
        from pathlib import Path as _P
        bus_file = _P("/tmp/trtc_stream/agent_events.jsonl")
        if not bus_file.parent.exists():
            return
        icons = {"search": "[SRCH]", "exec": "[EXEC]", "info": "[INFO]", "error": "[ERR]"}
        event = {
...
class MusicDownloader:
    ...
    def __init__(self):
        ...
        # 精简音源列表：去掉经常超时/不稳定的源（thttt SSL 常败, musicenc 常断连）
        # 保留 6 个稳定高效音源，按优先级排序
        self.sources = ['netease', 'kuwo', 'kugou', 'qq', 'gequbao', '9ku']
    
    # ============ 音源1: thttt.com ============
    def search_thttt(self, keyword: str) -> List[Dict]:
        try:
            url = f"https://www.thttt.com/so.php?wd={quote(keyword)}"
            resp = self.session.get(url, timeout=self.SEARCH_TIMEOUT, verify=False)
            html = resp.text
    ...
    def get_url_thttt(self, hash_code: str) -> Optional[str]:
        try:
            ...
            resp = self.session.post(url, data={'id': hash_code, 'type': 'dance'}, timeout=self.SEARCH_TIMEOUT, verify=False)
            result = resp.json()
    ...
    def search_kuwo(self, keyword: str) -> List[Dict]:
        try:
            ...
            resp = self.session.get(url, headers={"Referer": "https://www.kuwo.cn"}, timeout=self.SEARCH_TIMEOUT, verify=False)
            data = resp.json()
            songs = []
    ...
    def search_netease(self, keyword: str) -> List[Dict]:
        try:
            ...
            resp = self.session.get(url, headers=headers, timeout=self.SEARCH_TIMEOUT, verify=False)
            data = resp.json()
            songs = []
    ...
    def get_url_netease(self, song_id: str) -> Optional[str]:
        try:
            ...
            resp = self.session.get(url, timeout=self.SEARCH_TIMEOUT, verify=False, allow_redirects=False)
            ...
            resp = self.session.get(url, headers={"Referer": "https://music.163.com"}, timeout=self.SEARCH_TIMEOUT, verify=False)
            data = resp.json()
    ...
    def search_all(self, keyword: str) -> List[Dict]:
        ...
        with ThreadPoolExecutor(max_workers=6) as pool:
            futures = {pool.submit(func, keyword): name for name, func in sources}
            for future in as_completed(futures, timeout=self.SEARCH_TIMEOUT + 2):
                name = futures[future]
                try:
    ...
    def search_and_play(self, keyword: str) -> Optional[Dict]:
        """搜索并返回第一首可播放的歌曲信息（并行获取 URL）"""
        from concurrent.futures import ThreadPoolExecutor, as_completed
        ...
        for song in songs:
            ...
            if key in seen:
                continue
            seen.add(key)
            if song['source'] in get_url_funcs:
                candidates.append(song)
            if len(candidates) >= 6:
                break
        
        def _get_url(song):
            url = get_url_funcs[song['source']](song)
            if url:
                return {"artist": song["artist"], "name": song["name"], "source": song["source"], "url": url}
            return None
        
        with ThreadPoolExecutor(max_workers=6) as pool:
            futures = {pool.submit(_get_url, s): s for s in candidates}
            for future in as_completed(futures, timeout=self.URL_TIMEOUT + 2):
                try:
                    result = future.result(timeout=1)
                    if result:
                        print(f"\n▶️  {result['artist']} - {result['name']} ({result['source']})")
                        print(f"   {result['url']}")
                        # 取消其余任务
                        for f in futures:
                            f.cancel()
    ...
    def search_and_play(self, keyword: str) -> Optional[Dict]:
        """搜索并返回第一首可播放的歌曲信息（不下载）
        
        Returns:
            成功: {"artist": "...", "name": "...", "source": "...", "url": "..."}
            失败: None
        """
        _emit_dashboard("search", f"搜索音乐: {keyword}")
        print(f"\n🎵 搜索: {keyword}")
        print("=" * 40)
        ...
        if not songs:
            print("\n❌ 所有音源都未找到")
            _emit_dashboard("error", f"未找到歌曲: {keyword}")
            return None
        ...
        for song in songs:
            ...
            print(f"\n▶️  {result['artist']} - {result['name']} ({source})")
            print(f"   {play_url}")
            return result
...
def play_song(keyword: str) -> Optional[Dict]:
    """搜索并返回第一首可播放的歌曲 URL（不下载）"""
    downloader = MusicDownloader()
    return downloader.search_and_play(keyword)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("用法:")
        print("  python music_downloader.py '歌曲名'             # 默认: 返回播放 URL")
        print("  python music_downloader.py '歌曲名' --download  # 下载 MP3 文件")
        print("  python music_downloader.py '歌曲名' --json      # JSON 输出（便于脚本解析）")
    ...
    if mode == "--download":
        result = download_song(keyword)
        print(f"\n{'🎉 ' + result if result else '💥 下载失败'}")
    elif mode == "--json":
        result = play_song(keyword)
        if result:
            print(json.dumps(result, ensure_ascii=False))
        else:
            print(json.dumps({"error": "未找到可播放的歌曲"}, ensure_ascii=False))
    else:
        result = play_song(keyword)
        if result:
