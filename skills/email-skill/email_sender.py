...
class EmailSender:
    ...
    def load_config(self, config_path: str = None) -> dict:
        ...
        # Fall back to environment variables (only override when env var is actually set)
        env_mapping = {
        ...
        for cfg_key, env_key in env_mapping.items():
            ...
            if val is not None:
                if cfg_key == 'smtp_port':
                    ...
                    config[cfg_key] = val.lower() == 'true'
                else:
                    config[cfg_key] = val
        
        # Apply defaults only for keys not yet set
        defaults = {
            'smtp_port': 587,
    ...
    def send_email(
        ...
        try:
            ...
            # 使用 sendmail 而非 send_message，兼容 163/QQ 等国内邮箱
            # send_message 会重新解析 From 头，部分 SMTP 服务器不兼容
            server.sendmail(
